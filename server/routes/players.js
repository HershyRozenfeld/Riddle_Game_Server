...
router.get('/player/:username', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch player data', error: err.message });
  }
});
...
router.post('/submit-score', async (req, res) => {
  ...
  if (!username || !riddle_id || time_to_solve === undefined) {
    return res.status(400).json({ message: 'Missing required fields: username, riddle_id, time_to_solve' });
  }
